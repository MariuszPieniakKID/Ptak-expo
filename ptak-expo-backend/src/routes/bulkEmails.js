...
const processWelcomeEmailsJob = async (jobId, exhibition, exhibitors, loginUrl) => {
  ...
  for (let i = 0; i < exhibitors.length; i++) {
    ...
    try {
      ...
      const contactPerson = exhibitor.contact_person || '';
...
router.post('/send-welcome-by-exhibition', verifyToken, requireAdmin, async (req, res) => {
  try {
    ...
    processWelcomeEmailsJob(jobId, exhibition, result.rows, loginUrl).catch((err) => {

  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const res = await publicAPI.listExhibitorsByExhibition(id);
        const list = Array.isArray(res.data?.exhibitors) ? res.data.exhibitors : [];
        setRows(list);
      } catch (e: any) {
        setError(e?.message || 'Błąd ładowania wystawców');
      } finally {
        setLoading(false);
      }
    };
        ...
        {rows.map((ex) => (
          <div key={ex.exhibitor_id} style={{ border: '1px solid #eee', borderRadius: 12, padding: 16, marginBottom: 16 }}>
            <div style={{ display: 'flex', gap: 16, alignItems: 'center' }}>
              {ex.logo ? (
                <img src={ex.logo} alt={ex.name} style={{ width: 80, height: 80, objectFit: 'contain' }} />
              ) : null}
              <div>

...
import { formatDateRange, formatDateRangeDays } from "./utilities";
...
export const TradeInfoPage: React.FC<T_TradeInfoPage> = ({ eventId }) => {
  ...
  const mapBuildInformations = (
    tradeData?.buildDays && tradeData.buildDays.length > 0
      ? tradeData.buildDays
      : []
  ...
  console.log("[TradeInfoPage] Global construction events:", {
    ...
    globalConstructionEvents: globalConstructionEvents.map((ev) => ({
      id: ev.id,
      name: ev.name,
      exhibitor_id: ev.exhibitor_id,
      type: ev.type,
  ...
  const mapGlobalConstructionEvents = globalConstructionEvents.map(
  ...
  const handleDownloadPlan = useCallback(
    async (spaceId: string, filename: string) => {
      try {
        const exId = Number(eventId);
  ...
  const mapHall = (() => {
    ...
    const hallsWithMeta: Array<{
      id?: string;
      name: string;
      originalFilename?: string | null;
    }> =
      tradeData?.tradeSpaces && tradeData.tradeSpaces.length > 0
        ? tradeData.tradeSpaces.map((s: any) => ({
            id: String(s.id),
            name: s.hallName || s.name || "-",
    ...
    return hallsWithMeta.map((item, index: number) => {
      const isMine =
        my &&
        my.hallName &&
        String(item.name).toLowerCase() === String(my.hallName).toLowerCase();
      const isDownloadable = Boolean(item.id && item.originalFilename);
      const onClick = isDownloadable
        ? () =>
            handleDownloadPlan(
              item.id as string,
      ...
      return (
        <div
          ...
          title={
            isDownloadable ? `Pobierz – ${item.originalFilename}` : undefined
          }
        >
  ...
  const mapAllEvents = (() => {
    ...
    console.log(
      "[TradeInfoPage] Events with is_in_agenda:",
      allEvents.map((ev) => ({
        id: ev.id,
        name: ev.name,
        is_in_agenda: ev.is_in_agenda,
      }))
    );

    // Filter to show only events that are in agenda
    const agendaEvents = (allEvents || []).filter(
      (ev) => ev.is_in_agenda === true
    );
    ...
    return (
      <div style={{ display: "grid", gap: 12 }}>
        {items.map((ev) => (
          ...
          >
            <div style={{ color: "#A7A7A7", fontSize: 12 }}>
              {ev.event_date} • {String(ev.start_time).slice(0, 5)}–
              {String(ev.end_time).slice(0, 5)}
            </div>
            <div style={{ color: "#fff", fontWeight: 700 }}>{ev.name}</div>
            {ev.hall && (
              <div style={{ color: "#D7D9DD", fontSize: 12 }}>{ev.hall}</div>
            )}
            {ev.description && (
              <div style={{ color: "#D7D9DD", fontSize: 12 }}>
                {ev.description}
              </div>
            )}
          </div>
      ...
      <main className={styles.main}>
        ...
        <section className={styles.content}>
                ...
                <div className={styles.eventInformationsHoursItem}>
                  ...
                  <div className={styles.eventInformationsHoursItemDate}>
                    <CustomTypography
                      ...
                      color="#6F87F6"
                    >
                      {tradeData?.tradeHours
                        ? `${String(
                            tradeData.tradeHours.exhibitorStart || ""
                          ).slice(0, 5)} - ${String(
                            tradeData.tradeHours.exhibitorEnd || ""
                          ).slice(0, 5)}`
                        : "-"}
                    </CustomTypography>
                  </div>
                ...
                <div className={styles.eventInformationsHoursItem}>
                  ...
                  <div className={styles.eventInformationsHoursItemDate}>
                    ...
                    >
                      {tradeData?.tradeHours
                        ? `${String(
                            tradeData.tradeHours.visitorStart || ""
                          ).slice(0, 5)} - ${String(
                            tradeData.tradeHours.visitorEnd || ""
                          ).slice(0, 5)}`
                        : "-"}
                    </CustomTypography>
                  </div>
              ...
              <div className={styles.contactRow}>
                ...
                <div className={styles.contactMeta}>
                  <div className={styles.contactName}>
                    {tradeData?.exhibitorAssignment?.supervisor
                      ? `${tradeData.exhibitorAssignment.supervisor.firstName} ${tradeData.exhibitorAssignment.supervisor.lastName}`
                      : "-"}
                  </div>
          ...
          <div className={styles.contentGray}>
            <div className={styles.contentPosition}>
              ...
              >
                {tradeData?.exhibitorAssignment
                  ? `${tradeData.exhibitorAssignment.hallName || ""} / ${
                      tradeData.exhibitorAssignment.standNumber || ""
                    }`.trim()
                  : "-"}
              </CustomTypography>
            </div>
        ...
        <section className={styles.contentDark}>
          <CustomTypography
            ...
            fontWeight={700}
            color="white"
            className={styles.contentDarkHeader}
          >
            Plan Targów:
          </CustomTypography>
          ...
          <CustomTypography
            ...
            fontWeight={700}
            color="white"
            className={styles.accompanyingEvents}
          >
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 12 }}>
            <button
              ...
              style={{
                ...
                borderRadius: 6,
                border: 0,
                background: activeTab === "all" ? "#6f87f6" : "#2f2f35",
                color: "#fff",
              }}
            >
              Wszystkie

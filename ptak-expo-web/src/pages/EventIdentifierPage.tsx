...
import {brandingAPI, exhibitionsAPI, tradeInfoAPI} from "../services/api";
...
const EventIdentifierPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        const hallName =
          Array.isArray(trade?.tradeSpaces) && trade.tradeSpaces.length > 0
  ...
  return (
    <EventLayout
      ...
      right={<>{identifier && <IdentifierCard data={identifier} />}</>}
      colorLeft="#2E2E38"
    />
  );
};

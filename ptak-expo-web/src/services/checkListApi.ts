		...
		try {
			...
			if (r.ok) {
				...
				if (d) {
					...
					// also set email from catalog if present
					(ExampleChecklist.companyInfo as any).contactEmail = d.contact_email ?? (ExampleChecklist.companyInfo as any).contactEmail ?? null;
				}
			}
		} catch {}
...
export const addProduct = async (productInfo: ProductInfo) => {
	...
	const token = localStorage.getItem('authToken') || '';
	try {
		await fetch(`${config.API_BASE_URL}/api/v1/catalog/${exhibitionId}/products`, { method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` }, body: JSON.stringify(productInfo) });
	} catch {}
}

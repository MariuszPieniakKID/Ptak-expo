...
import { BrandingFilesResponse, Exhibition, getBrandingFiles } from "../../../../services/api";
import BrandingFileUpload from "../../../BrandingFileUpload";
...
function BrandingContent({ event }: BrandingContentProps) {
...
// Handle upload/delete success - reload files with correct context
  ...
  return (
    ...
    <Box className={styles.downloadMaterialcContainer}>
        ...
        {brandingError 
        && (<Alert severity="error" sx={{ mb: 2 }}>{brandingError}</Alert>
        )}
     {brandingLoading 
